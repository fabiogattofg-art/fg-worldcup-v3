...
const FG_V9=(()=>{
  ...
  function renderUltimate(){
    ...
    $("ultimateFanta").textContent=fTop?`${fTop.p.name} ${fTop.p.fantasyScore}`:"-";
...
const FG_EASY_UI=(()=>{
  ...
  function activeForEasy(m,team){
    ...
    if(names?.length)return names.map(n=>playerIndex?.byTeamNormName?.get(team)?.get(normName(n))).filter(Boolean);
    return active(team);
  }
  ...
  function dashboardCard(x){
    ...
    return `<article class="card easyCard ${signalClass(r.signal)}">
      ...
      <div class="signalRow"><span class="signal ${signalClass(r.signal)}">${r.label}</span><span class="miniBadge">${pick} &middot; ${pct(prob)}</span><span class="miniBadge">${fanta?`${fanta.name} ${fanta.fantasyScore}`:"Fanta -"}</span><span class="miniBadge">${flags.length} rischi</span></div>
      <div class="grid">
  ...
  function renderDashboard(){
    ...
    $("kpiBonus").textContent=fTop?`${fTop.p.name} ${fTop.p.fantasyScore}`:"-";
...
const FG_TICKET_INTEL=(()=>{
  ...
  function oddsPanel(p){
    ...
    const rows=marketsFor(p).map(market=>edgeForMarket(p.m,market,p)).sort(byFair);
    const saved=rows.filter(e=>e.odds),coreKeys=new Set();
    saved.forEach(e=>coreKeys.add(e.key));
    rows.filter(e=>!coreKeys.has(e.key)).slice(0,24).forEach(e=>coreKeys.add(e.key));
    const core=rows.filter(e=>coreKeys.has(e.key)).sort(byFair);
    const rest=rows.filter(e=>!coreKeys.has(e.key)).sort(byFair);
    return `<div class="marketHint">Scanner mercati: ${rows.length}. Ordinati per fair alta, dal mercato piu pagato in giu.</div><div class="oddsGrid">${core.map(e=>oddInputLine(e,p.m.id)).join("")}</div><details class="marketCatalog"><summary>Catalogo completo mercati (${rest.length})</summary><div class="oddsGrid">${rest.map(e=>oddInputLine(e,p.m.id)).join("")}</div></details>`;
  }
