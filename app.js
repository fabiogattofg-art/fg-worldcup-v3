...
function renderManualXI(){
  ...
  el.innerHTML = state.matches.slice(0,72).map(m => `
    <article class="card">
      <div class="cardHead">
        <div><div class="matchTitle">${matchTitle(m)}</div><div class="meta">${m.date} · ${m.lineupStatus || "XI ufficiale manuale"}</div></div>
      ...
      <button class="ghost" onclick="openImpactDetail(${m.id})">Dettaglio impatto</button>
