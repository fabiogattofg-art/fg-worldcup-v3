...
clearSimulationCache();recalcOptimized();FG_V9.ensure();

const FG_ABSOLUTE_AUTO=(()=>{
  const ENDPOINT="/api/absolute-auto-context";
  let busy=false;
  function ensure(){
    if(!state.absoluteAuto||typeof state.absoluteAuto!=="object")state.absoluteAuto={contexts:{}};
    if(!state.absoluteAuto.contexts)state.absoluteAuto.contexts={};
    FG_V9.ensure();
  }
  function ctxFor(m){ensure();return state.absoluteAuto.contexts[m.id]||null}
  function escText(v){return String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c]))}
  function knownPlayers(m){
    return playersForMatch(m).map(p=>({
      name:p.name,team:p.team,role:p.role,status:p.status,bonus:p.bonus,valueM:p.valueM,
      probableXI:p.probableXI||p.status==="Titolare",goals:p.goals,yellow:p.yellow
    }));
  }
  function sourceConfidence(ctx){return ctx?.ok?Number(ctx.confidence||0):0}
  function sourceLabel(ctx){
    if(!ctx)return "Da aggiornare";
    if(!ctx.ok)return "Auto off";
    return `${sourceConfidence(ctx)}/100`;
  }
  function log(msg){const el=$("absoluteLog");if(el)el.textContent=msg}
  function setBusy(v){
    busy=v;
    ["refreshAbsoluteVisible","refreshAbsoluteAll"].forEach(id=>{const b=$(id);if(b)b.disabled=v});
  }
  function applyAutoLineups(m,out){
    const lineup=out?.lineups,players=lineup?.players||[],counts=lineup?.counts||{};
    if(!players.length||counts.homeXI<10||counts.awayXI<10)return false;
    const before=getPreLineupSnapshot(m);
    const previous=playersForMatch(m).map(p=>({p,status:p.status,source:p.source}));
    const previousMatchLineups=m.matchLineups;
    playersForMatch(m).forEach(p=>{p.status="Panchina";p.source=p.source||"Squad DB"});
    m.matchLineups={home:{starters:[],bench:[],absent:[]},away:{starters:[],bench:[],absent:[]}};
    let homeXI=0,awayXI=0,homeBench=0,awayBench=0,outCount=0;
    for(const item of players){
      const p=playerIndex.byTeamNormName.get(item.team)?.get(normName(item.name));
      if(!p)continue;
      p.status=item.status==="Titolare"?"Titolare":item.status==="Fuori"?"Fuori":"Panchina";
      p.source=lineup.source||"SofaScore lineups";
      const side=p.team===m.home?"home":p.team===m.away?"away":null;
      if(side){
        const bucket=p.status==="Titolare"?"starters":p.status==="Fuori"?"absent":"bench";
        m.matchLineups[side][bucket].push(p.name);
      }
      if(p.team===m.home&&p.status==="Titolare")homeXI++;
      if(p.team===m.away&&p.status==="Titolare")awayXI++;
      if(p.team===m.home&&p.status==="Panchina")homeBench++;
    ...
    if(homeXI<10||awayXI<10){
      m.matchLineups=previousMatchLineups;
      previous.forEach(x=>{x.p.status=x.status;x.p.source=x.source});
      rebuildIndexes();
      return false;
    }
    m.sofascoreEventId=lineup.eventId||m.sofascoreEventId||null;
    m.lineupStatus=homeXI>=11&&awayXI>=11?"Auto XI SofaScore":"Auto XI parziale";
    m.lastAutoUpdate=new Date().toLocaleString();
    m.source=lineup.source||"Absolute Auto";
    rebuildIndexes();
    recalcAffectedTeams([m.home,m.away]);
    const after=getPreLineupSnapshot(m);
    m.lineupImpact={before,after,counts:{homeXI,awayXI,homeBench,awayBench,outCount}};
  ...
  function sideWeight(sig){
    const w=Number(sig.weight||0),c=clamp(Number(sig.confidence||.55),.2,1);
    if(sig.side==="home")return w*c;
    if(sig.side==="away")return -w*c;
    return 0;
  }
  function signalModel(m){
    const ctx=ctxFor(m),signals=ctx?.signals||[];
    const source=ctx?.ok?clamp(sourceConfidence(ctx)/100,.28,1):0;
    const rawEdge=signals.reduce((sum,s)=>sum+sideWeight(s),0);
    const risk=signals.reduce((sum,s)=>{
      const w=Number(s.weight||0),c=clamp(Number(s.confidence||.55),.2,1);
      const typeRisk=["weather","referee","pressure","trap"].includes(s.type) ? .035 : 0;
      return sum+(w<0?Math.abs(w)*c:0)+typeRisk;
    },0);
    const chaos=signals.reduce((sum,s)=>{
      const c=clamp(Number(s.confidence||.55),.2,1);
      return sum+(["weather","referee","pressure","trap","lineup"].includes(s.type)?c*.025:0);
    },0);
    return {ctx,signals,edge:clamp(rawEdge*source,-.18,.18),risk:clamp(risk,0,.44),chaos:clamp(chaos,0,.22),source};
  }
  function adjustedProbs(m){
    const s=m.sim||mc(m),base=s?.probs;
    if(!base)return null;
    const model=signalModel(m);
    let home=clamp(base.homeWin+model.edge,.02,.92);
    let away=clamp(base.awayWin-model.edge,.02,.92);
    let draw=clamp(base.draw+model.chaos*.12+model.risk*.055-Math.abs(model.edge)*.18,.04,.55);
    const total=home+draw+away||1;
    return {homeWin:home/total,draw:draw/total,awayWin:away/total,model};
  }
  function bestAdjustedPick(m){
    const p=adjustedProbs(m);
    if(!p)return {label:"-",prob:0};
    return [
      {label:m.home,prob:p.homeWin,key:"home"},
      {label:"X",prob:p.draw,key:"draw"},
      {label:m.away,prob:p.awayWin,key:"away"}
    ].sort((a,b)=>b.prob-a.prob)[0];
  }
  function autoFlags(m){
    const ctx=ctxFor(m),model=signalModel(m),flags=[...FG_V9.redFlagsFor(m)];
    if(!ctx)flags.unshift({type:"weak",label:"Auto mancante",detail:"Contesto automatico non ancora letto."});
    else if(!ctx.ok)flags.unshift({type:"avoid",label:"Auto non disponibile",detail:ctx.error||"Backend auto non raggiungibile."});
    else if(sourceConfidence(ctx)<45)flags.push({type:"weak",label:"Fonte debole",detail:`Affidabilita fonte ${sourceConfidence(ctx)}/100.`});
    if(model.risk>.20)flags.push({type:"trap",label:"Rischio live",detail:"News e contesto aumentano la fragilita della lettura."});
    if(model.chaos>.07)flags.push({type:"chaos",label:"Contesto caotico",detail:"Clima, arbitro o pressione possono sporcare la partita."});
    (ctx?.signals||[]).filter(s=>Number(s.weight||0)<-.045).slice(0,3).forEach(s=>{
      flags.push({type:s.type||"trap",label:s.label||"Segnale negativo",detail:s.detail||"Segnale automatico sfavorevole."});
    });
    const seen=new Set();
  ...
  function flagWeight(type){return type==="avoid"?7:type==="trap"?5:type==="weak"?4:type==="lowmargin"?3:type==="chaos"?3:type==="absence"?2:1}
  function absoluteScore(m){
    const base=FG_V9.ultimateScore(m),ctx=ctxFor(m),p=adjustedProbs(m),flags=autoFlags(m),model=signalModel(m);
    const sourceBonus=ctx?.ok?clamp(sourceConfidence(ctx)/100,0,1)*9:-8;
    const probs=p?[p.homeWin,p.draw,p.awayWin].sort((a,b)=>b-a):[0,0,0];
    const spread=(probs[0]||0)-(probs[1]||0);
    const clarity=clamp((spread-.10)*85,-7,15);
    const edgeBonus=Math.abs(model.edge)*22;
    const riskPenalty=model.risk*55+model.chaos*24+flags.reduce((sum,f)=>sum+flagWeight(f.type)*.9,0);
    const score=Math.round(clamp(base+sourceBonus+clarity+edgeBonus-riskPenalty,3,99));
    if(!ctx)return Math.min(score,74);
    if(!ctx.ok)return Math.min(score,58);
    return score;
  }
  function statusFor(m){
    const ctx=ctxFor(m),score=absoluteScore(m),flags=autoFlags(m);
    if(!ctx)return {label:"Da aggiornare",signal:"trap",score};
    if(!ctx.ok)return {label:"Auto non disponibile",signal:"avoid",score};
    if(score>=84&&flags.filter(f=>["avoid","trap","weak","chaos"].includes(f.type)).length<=1)return {label:"Lettura forte",signal:"playable",score};
    if(flags.some(f=>f.type==="avoid"||f.type==="weak"))return {label:"Fonte fragile",signal:"avoid",score};
    if(flags.some(f=>f.type==="trap"||f.type==="lowmargin"||f.type==="chaos"))return {label:"Buona ma fragile",signal:"trap",score};
    return {label:"Giocabile auto",signal:"playable",score};
  }
  function classFor(signal){return signal==="avoid"?"avoid":signal==="trap"?"trap":signal==="upset"?"upset":"playable"}
  function signalList(ctx){
    const list=ctx?.signals||[];
    if(!list.length)return `<div class="explainList"><p>Nessun segnale automatico forte.</p></div>`;
    return `<div class="explainList">${list.slice(0,5).map(s=>`<p><b>${escText(s.label||"Segnale")}</b> &middot; ${escText(s.detail||"")}</p>`).join("")}</div>`;
  }
  function sourceList(ctx){
    if(!ctx?.sources?.length)return `<div class="sourceList"><p>Fonti auto non ancora lette.</p></div>`;
    return `<div class="sourceList">${ctx.sources.slice(0,4).map(s=>`<p><b>${escText(s.label||"Fonte")}</b> &middot; ${s.ok?"ok":"debole"} &middot; ${escText(s.detail||"")}</p>`).join("")}</div>`;
  }
  function adjustedFantaTop(m){
    const ctx=ctxFor(m),signals=ctx?.signals||[];
    return topBonus(m).map(p=>{
      const key=normName(p.name),txt=signals.map(s=>`${s.label||""} ${s.detail||""}`).join(" ");
      const hit=normName(txt).includes(key);
      const impact=hit?signals.reduce((sum,s)=>{
        const line=normName(`${s.label||""} ${s.detail||""}`);
        return sum+(line.includes(key)?Number(s.weight||0)*Number(s.confidence||.55)*170:0);
      },0):0;
      return {...p,absoluteFantasyScore:Math.round(clamp((p.fantasyScore||0)+impact,1,99))};
    }).sort((a,b)=>b.absoluteFantasyScore-a.absoluteFantasyScore)[0]||null;
  }
  function absoluteCard(m){
    const ctx=ctxFor(m),p=adjustedProbs(m),pick=bestAdjustedPick(m),r=statusFor(m),flags=autoFlags(m),f=adjustedFantaTop(m);
    const lineups=ctx?.lineups?.counts,updated=ctx?.updatedAt?new Date(ctx.updatedAt).toLocaleString():"-";
    return `<article class="card v9Card absoluteCard ${classFor(r.signal)}">
      <div class="cardHead"><div><div class="matchTitle">${matchTitle(m)}</div><div class="meta">#${m.id} &middot; ${m.date} &middot; ${m.stage} ${m.group||""}</div>${contextBadges(m)}</div><div class="score ${cls(r.score)}">${r.score}</div></div>
      <div class="signalRow"><span class="signal ${classFor(r.signal)}">${r.label}</span><span class="miniBadge">Auto ${sourceLabel(ctx)}</span><span class="miniBadge">${pick.label} &middot; ${pct(pick.prob)}</span><span class="miniBadge">${lineups?`XI ${lineups.homeXI}/${lineups.awayXI}`:"XI auto -"}</span><span class="miniBadge">${flags.length} risk</span></div>
      <div class="confidenceBar" style="--w:${r.score}%"><span></span></div>
      <div class="grid">
        <div class="item"><span>1X2 Absolute</span><strong>${p?`1 ${pct(p.homeWin)} &middot; X ${pct(p.draw)} &middot; 2 ${pct(p.awayWin)}`:"-"}</strong></div>
        <div class="item"><span>Pick automatico</span><strong>${pick.label} &middot; ${pct(pick.prob)}</strong></div>
        <div class="item"><span>V9 base</span><strong>${FG_V9.ultimateScore(m)} &middot; ${FG_V9.reliability(m).label}</strong></div>
        <div class="item"><span>Fanta corretto</span><strong>${f?`${f.name} &middot; ${f.team} &middot; ${f.absoluteFantasyScore}`:"-"}</strong></div>
        <div class="item"><span>Fonte</span><strong>${sourceLabel(ctx)} &middot; ${updated}</strong></div>
        <div class="item"><span>Scenario</span><strong>${ctx?.summary?escText(ctx.summary):"Auto context non ancora letto."}</strong></div>
        <div class="item full"><span>Segnali automatici</span>${signalList(ctx)}</div>
        <div class="item full"><span>Red flags finali</span><div class="explainList">${(flags.length?flags:[{label:"Pulita",detail:"Nessuna red flag forte."}]).map(x=>`<p><b>${escText(x.label)}</b> &middot; ${escText(x.detail)}</p>`).join("")}</div></div>
        <div class="item full"><span>Fonti lette</span>${sourceList(ctx)}</div>
      </div>
      <button class="ghost detailBtn" onclick="FG_ABSOLUTE_AUTO.refreshMatch(${m.id})">Aggiorna auto</button>
    </article>`;
  }
  function filteredAbsoluteMatches(){
    const q=$("absoluteSearch")?.value?.toLowerCase()||"",filter=$("absoluteFilter")?.value||"";
    return state.matches.filter(m=>{
      const text=matchTitle(m).toLowerCase(),ctx=ctxFor(m),flags=autoFlags(m),r=statusFor(m);
      const passText=!q||text.includes(q)||m.home.toLowerCase().includes(q)||m.away.toLowerCase().includes(q);
      const passFilter=!filter||
        (filter==="ready"&&ctx?.ok)||
        (filter==="clean"&&r.signal==="playable"&&r.score>=82)||
        (filter==="trap"&&(r.signal==="trap"||flags.some(f=>["trap","lowmargin"].includes(f.type))))||
        (filter==="chaos"&&flags.some(f=>["chaos","weather","referee","pressure"].includes(f.type)))||
        (filter==="weak"&&(!ctx||!ctx.ok||sourceConfidence(ctx)<45));
      return passText&&passFilter;
    });
  }
  function renderAbsolute(){
    const el=$("absoluteCards");if(!el)return;
    ensure();
    const all=state.matches.map(m=>({m,score:absoluteScore(m),flags:autoFlags(m),ctx:ctxFor(m),r:statusFor(m)}));
    const bestReady=[...all].filter(x=>x.ctx?.ok).sort((a,b)=>b.score-a.score)[0];
    const best=bestReady||[...all].sort((a,b)=>b.score-a.score||FG_V9.ultimateScore(b.m)-FG_V9.ultimateScore(a.m))[0];
    const ready=all.filter(x=>x.ctx?.ok).length;
    const avgSource=ready?Math.round(all.filter(x=>x.ctx?.ok).reduce((s,x)=>s+sourceConfidence(x.ctx),0)/ready):0;
    $("absolutePick").textContent=bestReady?`${matchTitle(bestReady.m)} ${bestReady.score}`:"Aggiorna auto";
    $("absoluteLive").textContent=ready;
    $("absoluteRisk").textContent=Math.round(all.reduce((s,x)=>s+x.flags.filter(f=>["avoid","trap","weak","chaos"].includes(f.type)).length,0)/Math.max(1,all.length)*10);
    $("absoluteSource").textContent=ready?`${avgSource}/100`:"-";
    const list=filteredAbsoluteMatches().sort((a,b)=>absoluteScore(b)-absoluteScore(a));
    el.innerHTML=list.length?list.map(absoluteCard).join(""):`<article class="card absoluteCard"><div class="matchTitle">Nessuna partita trovata</div><p>Cambia filtro o cerca un'altra squadra.</p></article>`;
  }
  async function refreshMatch(id,opts={}){
    ensure();
    const m=state.matches.find(x=>x.id===id);
    if(!m)return null;
    if(!opts.batch)log(`Auto context: leggo ${matchTitle(m)}...`);
    try{
      const r=await fetch(ENDPOINT,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({home:m.home,away:m.away,date:m.date,eventId:m.sofascoreEventId||null,knownPlayers:knownPlayers(m)})});
      if(!r.ok)throw new Error("Backend auto non disponibile");
      const out=await r.json();
      out.appliedLineups=out.ok&&applyAutoLineups(m,out);
      out.localUpdatedAt=new Date().toISOString();
      state.absoluteAuto.contexts[m.id]=out;
      state.absoluteAuto.lastBatch=out.localUpdatedAt;
      save();
      if(!opts.batch)log(out.ok?`Auto context aggiornato: ${matchTitle(m)}.`:`Auto context debole: ${out.error||matchTitle(m)}.`);
      renderAbsolute();
      return out;
    }catch(e){
      const out={ok:false,source:"Absolute Auto",error:e.message||"Errore auto",updatedAt:new Date().toISOString(),signals:[],sources:[]};
      state.absoluteAuto.contexts[m.id]=out;
      save();
      if(!opts.batch)log(`Auto context non raggiungibile per ${matchTitle(m)}.`);
      renderAbsolute();
      return out;
    }
  }
  async function refreshList(list,label){
    if(busy)return;
    setBusy(true);
    try{
      const targets=list.slice(0,12);
      if(!targets.length){log("Nessuna partita da aggiornare.");return}
      for(let i=0;i<targets.length;i++){
        log(`${label}: ${i+1}/${targets.length} ${matchTitle(targets[i])}`);
        await refreshMatch(targets[i].id,{batch:true});
      }
      log(`${label}: completato su ${targets.length} partite.`);
    }finally{
      setBusy(false);
      renderAbsolute();
    }
  ...
  function refreshVisible(){
    let list=filteredAbsoluteMatches().sort((a,b)=>absoluteScore(b)-absoluteScore(a));
    if(!list.length)list=[...state.matches].sort((a,b)=>FG_V9.ultimateScore(b)-FG_V9.ultimateScore(a));
    return refreshList(list.slice(0,8),"Auto visibili");
  }
  function refreshTop12(){
    const list=[...state.matches].sort((a,b)=>absoluteScore(b)-absoluteScore(a)||FG_V9.ultimateScore(b)-FG_V9.ultimateScore(a));
    return refreshList(list.slice(0,12),"Auto top 12");
  }
  function renderRouter(tabId=activeTabId()){
    if(tabId==="absolute")return renderAbsolute();
    return FG_V9.renderV9(tabId);
  }
  const api={ensure,ctxFor,absoluteScore,adjustedProbs,autoFlags,statusFor,renderAbsolute,refreshMatch,refreshVisible,refreshTop12,renderRouter};
  if(typeof window!=="undefined")window.FG_ABSOLUTE_AUTO=api;
  return api;
})();

render=FG_ABSOLUTE_AUTO.renderRouter;
if($("absoluteSearch"))$("absoluteSearch").oninput=FG_ABSOLUTE_AUTO.renderAbsolute;
if($("absoluteFilter"))$("absoluteFilter").onchange=FG_ABSOLUTE_AUTO.renderAbsolute;
if($("refreshAbsoluteVisible"))$("refreshAbsoluteVisible").onclick=FG_ABSOLUTE_AUTO.refreshVisible;
if($("refreshAbsoluteAll"))$("refreshAbsoluteAll").onclick=FG_ABSOLUTE_AUTO.refreshTop12;
FG_ABSOLUTE_AUTO.ensure();
render();
