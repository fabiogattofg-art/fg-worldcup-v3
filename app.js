...
function renderOfficial(){
  ...
  el.innerHTML = list.map(m => {
    const homeXI = state.players.filter(p=>p.team===m.home && p.status==="Titolare").length;
    const awayXI = state.players.filter(p=>p.team===m.away && p.status==="Titolare").length;
    const homeBench = state.players.filter(p=>p.team===m.home && p.status==="Panchina").length;
    const awayBench = state.players.filter(p=>p.team===m.away && p.status==="Panchina").length;
    return `<article class="card">
      <div class="cardHead">
...
function applyManualXI(id){
  ...
  if(before && typeof getPreLineupSnapshot === "function"){
    const after = getPreLineupSnapshot(m);
    m.lineupImpact = {before, after, counts:{homeXI:11, awayXI:11, homeBench:0, awayBench:0, outCount:state.players.filter(p=>p.team===m.home||p.team===m.away).length-22}};
    save();
  }
...
function render(){renderManualXI();renderToday();renderOfficial();renderMoney();renderFantasy();renderSquads();renderMC();renderLineups();renderBest()}
