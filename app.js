...
function probBars(m){
  const s = m.sim;
  if(!s) return "";
  return `<div class="probBars">
    <div class="probRow"><span>${m.home}</span><div class="barTrack"><div class="barFill" style="width:${Math.round(s.probs.homeWin*100)}%"></div></div><b>${pct(s.probs.homeWin)}</b></div>
    <div class="probRow"><span>X</span><div class="barTrack"><div class="barFill" style="width:${Math.round(s.probs.draw*100)}%"></div></div><b>${pct(s.probs.draw)}</b></div>
    <div class="probRow"><span>${m.away}</span><div class="barTrack"><div class="barFill" style="width:${Math.round(s.probs.awayWin*100)}%"></div></div><b>${pct(s.probs.awayWin)}</b></div>
  </div>`;
}
function renderTopOpportunities(){
  const el = $("topOpportunities");
  if(!el) return;
  const list = filtered ? filtered().filter(m=>m.sim && m.aiPicks?.safe) : state.matches.filter(m=>m.sim && m.aiPicks?.safe);
  const safe = [...list].sort((a,b)=>b.aiPicks.safe.prob-a.aiPicks.safe.prob)[0];
  const value = [...list].filter(m=>m.aiPicks?.value).sort((a,b)=>b.aiPicks.value.prob-a.aiPicks.value.prob)[0];
  let fantasy = [];
  list.forEach(m=>topBonus(m).forEach(p=>fantasy.push({...p, match:matchTitle(m)})));
  fantasy.sort((a,b)=>b.fantasyScore-a.fantasyScore);
  const cap = fantasy[0];

  el.innerHTML = `
    <div class="tradeTile"><span>💰 Safe Bet</span><strong>${safe?matchTitle(safe):"-"}</strong><div class="big safe">${safe?safe.aiPicks.safe.label:"-"}</div><p>${safe?pct(safe.aiPicks.safe.prob)+" · fair "+safe.aiPicks.safe.fair:""}</p></div>
    <div class="tradeTile"><span>📈 Top Value</span><strong>${value?matchTitle(value):"-"}</strong><div class="big value">${value?value.aiPicks.value.label:"-"}</div><p>${value?pct(value.aiPicks.value.prob)+" · quota min "+value.aiPicks.value.fair:""}</p></div>
    <div class="tradeTile"><span>🎯 Capitano AI</span><strong>${cap?cap.match:"-"}</strong><div class="big combo">${cap?cap.name:"-"}</div><p>${cap?"World Cup Score "+cap.fantasyScore:""}</p></div>
  `;
}

function snapshotMatch(m){
  const s=m.sim||mc(m); const starters=state.players.filter(p=>(p.team===m.home||p.team===m.away)&&p.status==="Titolare").map(p=>p.name);
...
function buildFormationImpact(m,before){
  ...
  const now=state.players.filter(p=>(p.team===m.home||p.team===m.away)&&p.status==="Titolare").map(p=>p.name);
...
async function updateOfficialLineup(id){
  const m = state.matches.find(x=>x.id===id);
  if(!m) return;
  const knownPlayers = state.players
    .filter(p=>p.team===m.home || p.team===m.away)
    .map(p=>({name:p.name, team:p.team, role:p.role}));
  ...
  try {
    ...
    state.players.filter(p=>p.team===m.home || p.team===m.away).forEach(p=>{
      p.status = "Panchina";
    ...
    for(const item of out.players){
      const p = state.players.find(x => x.team===item.team && normName(x.name)===normName(item.name));
      if(!p) continue;
...
function lineupStatusLabelForPlayer(p){
  if(p.status==="Titolare") return "TITOLARE";
  if(p.status==="Fuori") return "FUORI";
  return "NON FIGURA";
}
...
function renderManualXI(){
  ...
  if(!el) return;
  const q = ($("manualXiSearch")?.value || "").toLowerCase();
  const list = state.matches.slice(0,72).filter(m => !q || matchTitle(m).toLowerCase().includes(q) || (m.date||"").includes(q));
  el.innerHTML = list.map(m => {
    const homeXI = state.players.filter(p=>p.team===m.home && p.status==="Titolare").map(p=>p.name).join("
");
    const awayXI = state.players.filter(p=>p.team===m.away && p.status==="Titolare").map(p=>p.name).join("
");
    const impact = typeof renderImpactPanel === "function" ? renderImpactPanel(m) : "";
    return `<article class="card">
      <div class="cardHead">
        <div>
          <div class="matchTitle">${matchTitle(m)}</div>
          <div class="meta">${m.date} · ${m.lineupStatus || "In attesa XI ufficiale manuale"}</div>
        </div>
        <div class="score ${cls(m.fg)}">${m.fg || "TBD"}</div>
      </div>
      <div class="grid">
        <div class="item full">
          <span>${m.home} - 11 titolari</span>
          <textarea id="homeXI_${m.id}" placeholder="Scrivi 11 nomi, uno per riga o separati da virgola">${homeXI}</textarea>
        </div>
        <div class="item full">
          <span>${m.away} - 11 titolari</span>
          <textarea id="awayXI_${m.id}" placeholder="Scrivi 11 nomi, uno per riga o separati da virgola">${awayXI}</textarea>
        </div>
        <div class="item full"><span>Fonte attiva</span><strong>${m.lineupStatus==="Ufficiale manuale" ? "XI ufficiale manuale attivo" : "In attesa XI manuale"}</strong></div>
        ${impact}
      </div>
      <button onclick="applyManualXI(${m.id})">Applica XI ufficiale e ricalcola</button>
      <button class="ghost" onclick="openImpactDetail && openImpactDetail(${m.id})">Dettaglio impatto</button>
...
function applyManualXI(id){
  ...
  homeMatched.forEach(p=>{p.status="Titolare"; p.source="XI ufficiale manuale";});
  awayMatched.forEach(p=>{p.status="Titolare"; p.source="XI ufficiale manuale";});

  m.lineupStatus = "Ufficiale manuale";
  ...
  {
    const after = snapshotMatch(m);
    const removedHome = state.players.filter(p=>p.team===m.home && p.status!=="Titolare").map(p=>p.name);
    const removedAway = state.players.filter(p=>p.team===m.away && p.status!=="Titolare").map(p=>p.name);
    m.lineupImpact = {
      before,
      after,
      removed:{home:removedHome, away:removedAway},
      counts:{homeXI:11, awayXI:11, homeBench:0, awayBench:0, outCount:removedHome.length+removedAway.length}
    };
    save();
  }

  render();
  alert(`XI ufficiale manuale applicato e modello ricalcolato.\n${m.home}: 11 titolari\n${m.away}: 11 titolari\nIl resto non figura nel modello.`);
}

function fillCurrentXI(id){
  const m = state.matches.find(x=>x.id===id);
  if(!m) return;
  const home = state.players.filter(p=>p.team===m.home && p.status==="Titolare").map(p=>p.name).join("\\n");
  const away = state.players.filter(p=>p.team===m.away && p.status==="Titolare").map(p=>p.name).join("\\n");
  if($("homeXI_"+id)) $("homeXI_"+id).value = home;
  if($("awayXI_"+id)) $("awayXI_"+id).value = away;
}

function render(){renderTopOpportunities();renderManualXI();renderToday();renderOfficial();renderMoney();renderFantasy();renderSquads();renderMC();renderLineups();renderBest()}
...
function openLineups(id){const m=state.matches.find(x=>x.id===id);const block=t=>`<h3>${t}</h3>${state.players.filter(p=>p.team===t).map(p=>`<p><b>${p.name}</b> · ${p.role} · ${lineupStatusLabelForPlayer(p)} · ${p.club||""}</p>`).join("")}`;$("modalContent").innerHTML=`<h2>${matchTitle(m)}</h2>${block(m.home)}${block(m.away)}`;$("modal").hidden=false}
...
if($("manualXiSearch")) $("manualXiSearch").oninput=renderManualXI;fill();recalc();render();
