...
function renderSquadsOptimized(){const q=$("squadSearch")?.value?.toLowerCase()||"";let teamsList=playerIndex.teams.filter(t=>!q||t.toLowerCase().includes(q)||playersForTeam(t).some(p=>p.name.toLowerCase().includes(q))).slice(0,48);$("squadCards").innerHTML=teamsList.map(t=>{const ps=playersForTeam(t).filter(p=>!q||t.toLowerCase().includes(q)||p.name.toLowerCase().includes(q));return `<article class="card"><h3>${t}</h3><p>${ps.length} giocatori · XI: ${active(t).length}</p>${ps.slice(0,30).map(p=>`<span class="pill">${p.name} · ${p.role} · ${p.club||""} · ${p.status}</span>`).join(" ")}</article>`}).join("")}
...
function openLineupsOptimized(id){const m=state.matches.find(x=>x.id===id);const block=t=>`<h3>${t}</h3>${playersForTeam(t).map(p=>`<p><b>${p.name}</b> · ${p.role} · ${p.status} · ${p.club||""}</p>`).join("")}`;$("modalContent").innerHTML=`<h2>${matchTitle(m)}</h2>${block(m.home)}${block(m.away)}`;$("modal").hidden=false}
...
const FG_V7=(()=>{
  ...
  function openLineupsV7(id){
    const m=state.matches.find(x=>x.id===id);if(!m)return;
    const block=t=>{const starters=new Set(activeForMatch(m,t).map(p=>p.name));return `<h3>${t}</h3>${playersForTeam(t).map(p=>`<p><b>${p.name}</b> · ${p.role} · ${starters.has(p.name)?"Titolare partita":p.status} · ${p.club||""}</p>`).join("")}`};
...
const FG_V9=(()=>{
  ...
  function fantaLine(p){
    return `<p><b>${p.name}</b> · ${p.team} · ${p.role} · score ${p.fantasyScore}<br>${matchTitle(p.match)} · Goal ${pct(p.fanta.goalProb)}${p.role==="POR"?` · Porta inv. ${pct(p.fanta.cleanSheetProb)}`:""} · Cart. ${pct(p.fanta.cardRisk)}</p>`;
  }
...
const FG_EASY_UI=(()=>{
  ...
  function renderManage(){
    ...
    const teams=(playerIndex?.teams||[]).filter(t=>!qTeam||t.toLowerCase().includes(qTeam)||playersForTeam(t).some(p=>p.name.toLowerCase().includes(qTeam))).slice(0,10);
    const matches=state.matches.filter(m=>!qMatch||matchTitle(m).toLowerCase().includes(qMatch)||m.home.toLowerCase().includes(qMatch)||m.away.toLowerCase().includes(qMatch)).slice(0,12);
    ...
    $("manageCards").innerHTML=`
      ...
      <article class="card easyCard"><div class="matchTitle">Rose rapide</div><div class="easyList">${teams.map(t=>`<p><b>${safeText(t)}</b> &middot; ${playersForTeam(t).length} giocatori &middot; XI ${active(t).length}</p>`).join("")||"<p>Nessuna squadra trovata.</p>"}</div></article>
      <article class="card easyCard"><div class="matchTitle">Formazioni per partita</div><div class="easyList">${matches.map(m=>`<p><b>${matchTitle(m)}</b> &middot; ${safeText(m.lineupStatus||"XI base")} &middot; <button class="ghost miniAction" onclick="openLineups(${m.id})">Apri</button></p>`).join("")}</div></article>`;
...
const FG_TICKET_INTEL=(()=>{
  ...
  function marketsFor(p){
    ...
    const s=p.s,markets=[
      ...
      {key:"dnb_h",label:`${p.m.home} DNB`,prob:s.probs.homeWin,winProb:s.probs.homeWin,loseProb:s.probs.awayWin,pushProb:s.probs.draw,push:true,type:"handicap"},
      {key:"dnb_a",label:`${p.m.away} DNB`,prob:s.probs.awayWin,winProb:s.probs.awayWin,loseProb:s.probs.homeWin,pushProb:s.probs.draw,push:true,type:"handicap"},
      ...
      {key:"home_score",label:`${p.m.home} segna`,prob:1-s.probs.awayCS,type:"team"},
      {key:"away_score",label:`${p.m.away} segna`,prob:1-s.probs.homeCS,type:"team"},
      {key:"home_cs",label:`Clean sheet ${p.m.home}`,prob:s.probs.homeCS,type:"team"},
      {key:"away_cs",label:`Clean sheet ${p.m.away}`,prob:s.probs.awayCS,type:"team"},
      {key:"home_win_nil",label:`${p.m.home} vince a zero`,prob:probWhere(p,c=>c.h>c.a&&c.a===0),type:"combo"},
      {key:"away_win_nil",label:`${p.m.away} vince a zero`,prob:probWhere(p,c=>c.a>c.h&&c.h===0),type:"combo"},
      {key:"home_btts",label:`${p.m.home} + BTTS`,prob:probWhere(p,c=>c.h>c.a&&c.h>0&&c.a>0),type:"combo"},
      {key:"away_btts",label:`${p.m.away} + BTTS`,prob:probWhere(p,c=>c.a>c.h&&c.h>0&&c.a>0),type:"combo"},
      {key:"home_o15",label:`${p.m.home} + Over 1.5`,prob:probWhere(p,c=>c.h>c.a&&c.total>1.5),type:"combo"},
      {key:"away_o15",label:`${p.m.away} + Over 1.5`,prob:probWhere(p,c=>c.a>c.h&&c.total>1.5),type:"combo"},
      {key:"home_o25",label:`${p.m.home} + Over 2.5`,prob:probWhere(p,c=>c.h>c.a&&c.total>2.5),type:"combo"},
      {key:"away_o25",label:`${p.m.away} + Over 2.5`,prob:probWhere(p,c=>c.a>c.h&&c.total>2.5),type:"combo"},
    ...
    [-2.5,-1.5,-1,-.5,.5,1,1.5,2.5].forEach(line=>{
      ...
      markets.push({key,label:`${p.m.home} AH ${line>0?"+":""}${line}`,prob:win,winProb:win,loseProb:lose,pushProb:push,push:push>0,type:"handicap"});
    });
    [-2.5,-1.5,-1,-.5,.5,1,1.5,2.5].forEach(line=>{
      const key=`ah_a_${String(line).replace("-","m").replace(".","p")}`;
      const win=probWhere(p,c=>-c.margin+line>0),push=Number.isInteger(line)?probWhere(p,c=>-c.margin+line===0):0,lose=1-win-push;
      markets.push({key,label:`${p.m.away} AH ${line>0?"+":""}${line}`,prob:win,winProb:win,loseProb:lose,pushProb:push,push:push>0,type:"handicap"});
    });
    [0.5,1.5,2.5,3.5].forEach(line=>{
      markets.push({key:`home_tt_o${String(line).replace(".","p")}`,label:`${p.m.home} gol Over ${line}`,prob:probWhere(p,c=>c.h>line),type:"team"});
      markets.push({key:`home_tt_u${String(line).replace(".","p")}`,label:`${p.m.home} gol Under ${line}`,prob:probWhere(p,c=>c.h<line),type:"team"});
      markets.push({key:`away_tt_o${String(line).replace(".","p")}`,label:`${p.m.away} gol Over ${line}`,prob:probWhere(p,c=>c.a>line),type:"team"});
      markets.push({key:`away_tt_u${String(line).replace(".","p")}`,label:`${p.m.away} gol Under ${line}`,prob:probWhere(p,c=>c.a<line),type:"team"});
    });
  ...
  function oddsPanel(p){
    ...
    rows.filter(e=>!coreKeys.has(e.key)).sort((a,b)=>b.score-a.score||b.scoreProb-a.scoreProb).slice(0,24).forEach(e=>coreKeys.add(e.key));
    const core=rows.filter(e=>coreKeys.has(e.key)).sort((a,b)=>(b.odds?1:0)-(a.odds?1:0)||b.valueScore-a.valueScore||b.score-a.score);
    const rest=rows.filter(e=>!coreKeys.has(e.key));
    return `<div class="marketHint">Scanner mercati: ${rows.length}. In alto i piu utili, sotto il catalogo completo.</div><div class="oddsGrid">${core.map(e=>oddInputLine(e,p.m.id)).join("")}</div><details class="marketCatalog"><summary>Catalogo completo mercati (${rest.length})</summary><div class="oddsGrid">${rest.map(e=>oddInputLine(e,p.m.id)).join("")}</div></details>`;
  }
