...
const FG_TICKET_INTEL=(()=>{
  ...
  function bestMarket(m,p){
    ...
    const s=p.s;
    const markets=[
      {label:"1",prob:s.probs.homeWin,type:"1x2"},
      {label:"X",prob:s.probs.draw,type:"1x2"},
      {label:"2",prob:s.probs.awayWin,type:"1x2"},
      {label:"Over 1.5",prob:s.probs.over15,type:"goals"},
      {label:"Over 2.5",prob:s.probs.over25,type:"goals"},
      {label:"Under 2.5",prob:s.probs.under25,type:"goals"},
      {label:"Under 4.5",prob:s.probs.under45,type:"goals"},
      {label:"BTTS Si",prob:s.probs.btts,type:"goals"}
    ].map(x=>({...x,score:marketStrength(x,p),fair:fair(x.prob)}));
  ...
  function adviceFromProfile(p){
    ...
    const reasons=[
      `Pick meno sporco: ${market.label} al ${fmtPct(market.prob)}.`,
    ...
    return {grade,label,score,pick:grade==="avoid"?"Passa":market.label,prob:grade==="avoid"?0:market.prob,risk:flags[0]?.label||(p.drawLive?"Pareggio vivo":"Pulita"),reasons:reasons.slice(0,4),risks:risks.slice(0,4)};
  }
  ...
  return {renderMoney,adviceFromProfile,profile};
})();
